...
onAuthStateChanged(auth, async (user) => {
  ...
  postBtn.onclick = async () => {
    ...
    try {
      ...
      // Get user profile for displayName and photo
    ...
    snapshot.forEach((docSnap) => {
      ...
      post.innerHTML = `
        <div class="postHeader">
          <img src="${data.photoURL || 'default-avatar.png'}" alt="avatar" class="avatar">
          <strong>${data.displayName}</strong>
        </div>
        <p>${data.text || ""}</p>
        ${data.postImage ? `<img src="${data.postImage}" class="postMedia">` : ""}
        ${data.postVideo ? `<video controls class="postMedia"><source src="${data.postVideo}"></video>` : ""}
      ...
      (data.comments || []).forEach(c => {
        ...
        cEl.textContent = `${c.displayName}: ${c.text}`;
