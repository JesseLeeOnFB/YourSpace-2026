...
onAuthStateChanged(auth, async (user) => {
  ...
  postBtn.onclick = async () => {
    ...
    try {
      ...
      const postDoc = await addDoc(collection(db, "posts"), {
        ...
        dislikes: 0,
        comments: [],
        mentions: [], // future: array of tagged usernames
      ...
      // Send notifications if checked (mock)
      if (notifyBrowser.checked) {
        new Notification("YourSpace", { body: "Your post was created!" });
      }
      if (notifyEmail.checked) {
        console.log("Email notification logic here for post:", postDoc.id);
      }
  ...
  onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      const mediaHTML = data.postImage ? `<img src="${data.postImage}" class="postImage">` :
                        data.postVideo ? `<video src="${data.postVideo}" class="postVideo" controls></video>` : "";

      post.innerHTML = `
        <div class="postHeader">
          <img src="${data.photoURL || 'default-avatar.png'}" class="postProfilePic" style="width:40px;height:40px;border-radius:50%;">
