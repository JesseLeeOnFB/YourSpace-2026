...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onSnapshot(postsQuery, (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
      ...
      // Add post with like/comment/share buttons
      postDiv.innerHTML = `
      ...
      postDiv.querySelector(".deleteBtn").addEventListener("click", async () => {
        if (auth.currentUser.uid !== data.userId) return alert("Cannot delete other people's posts");
        await updateDoc(doc(db, "posts", docSnap.id), { deleted: true });
      });

      // Comment and share buttons placeholder (can expand later)
      postDiv.querySelector(".commentBtn").addEventListener("click", () => { alert("Comment feature coming soon"); });
      postDiv.querySelector(".shareBtn").addEventListener("click", () => { alert("Share feature coming soon"); });

      postsContainer.appendChild(postDiv);
    });
  });

  // Trending Post - top likes in last hour
  setInterval(async () => {
