  ...
  onSnapshot(postsQuery, (snapshot) => {
    ...
    snapshot.docs
      ...
      .forEach((docSnap) => {
        ...
        let contentHTML = `<strong>${data.displayName}</strong>`;
        if (data.text) contentHTML += `<p>${data.text}</p>`;
        if (data.postImage) contentHTML += `<img src="${data.postImage}" class="postMedia">`;
        if (data.postVideo) contentHTML += `<video src="${data.postVideo}" class="postMedia" controls></video>`;
        ...
        postDiv.querySelector(".commentBtn").onclick = async () => {
          ...
          await updateDoc(doc(db, "posts", docSnap.id), { comments: arrayUnion(newComment) });
        };
        ...
        if (deleteBtn) {
          deleteBtn.onclick = async () => {
            if (confirm("Delete this post?")) await updateDoc(doc(db, "posts", docSnap.id), { deleted: true });
          };
        }
        ...
        if (data.comments?.length) {
          commentsSection.innerHTML = data.comments
            .map(c => `<p><strong>${c.displayName}:</strong> ${c.text}</p>`)
            .join("");
        }
      });
  });
