...
async function loadProfile(uid) {
  ...
  profileName.textContent = userData.username || "YourSpace User";
  profileBio.textContent = userData.bio || "This is the bio lol";
  profilePic.src = userData.profilePicture || "default-avatar.png";
...
async function loadTop10(uid) {
  ...
  friends.forEach(f => {
    ...
    div.innerHTML = `<img src="${f.pfpURL || 'default-avatar.png'}"><span>${f.username || 'Unknown'}</span>`;
    div.onclick = () => {
...
async function loadMusic(uid) {
  ...
  playlist.forEach((song, i) => {
    ...
    btn.textContent = song.title || song;
    btn.onclick = () => {
      ...
      musicPlayerIframe.src = embedURL;
    };
    musicPlaylistContainer.appendChild(btn);
  });
}
...
addMusicBtn?.addEventListener("click", async () => {
  ...
  const playlist = snap.data()?.musicPlaylist || [];
  playlist.push({ url: musicInput.value.trim(), title: musicInput.value.trim() });
