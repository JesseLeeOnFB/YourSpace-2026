...
const RATE_LIMITS = {
// Posts - UPDATED: More lenient (5 posts per 2 minutes)
POST_PER_2_MINUTES: 5,
POST_PER_HOUR: 15,
POST_PER_DAY: 75,
...
- @returns {Promise<{allowed: boolean, reason: string}>}
  */
  ...
  actionHistory.last2Minutes = (actionHistory.last2Minutes || []).filter(t => now - t < twoMinutes);
  ...
  switch(actionType) {
  case ‘post’:
  if (actionHistory.last2Minutes.length >= RATE_LIMITS.POST_PER_2_MINUTES) {
  limitExceeded = true;
  const waitTime = Math.ceil((actionHistory.last2Minutes[0] + twoMinutes - now) / 1000);
  reason = `Slow down! You've posted ${RATE_LIMITS.POST_PER_2_MINUTES} times in 2 minutes. Please wait ${waitTime} seconds.`;
  } else if (actionHistory.lastHour.length >= RATE_LIMITS.POST_PER_HOUR) {
  limitExceeded = true;
  reason = “You’ve reached the hourly post limit (15 posts/hour). Please try again later.”;
  } else if (actionHistory.lastDay.length >= RATE_LIMITS.POST_PER_DAY) {
  limitExceeded = true;
  reason = “You’ve reached the daily post limit (75 posts/day). Please try again tomorrow.”;
  }
  break;
  ...
  actionHistory.last2Minutes.push(now);
